...
public class Poster {
	...
	public Taxonomy addNicheCategory() {
		...
		try {
			...
			if (nicheCategory == null) {
				...
				nicheCategory = wpManager.saveOrUpdateTaxonomy(Taxonomy
						.createCategory(category, slug, desc));
			}
		} catch (ServiceException ex) {
	...
	List<Taxonomy> addNicheTags(final String... keywords) {
		...
		Set<String> tags = new HashSet<String>(
				ContentUtil.getTagsFromString(niche));
		...
		try {
			for (String tag : tags) {
				...
				if (tag.endsWith("s")) {
					taxonomyTag = wpManager.getTaxonomyTag(tag.substring(0,
							tag.length() - 2));
					LOG.debug(String.format("'%s' - plural tag %s", tag,
							taxonomyTag));
				}
				if (taxonomyTag == null) {
	...
	public void setWpManager(WPManager wpManager) {
